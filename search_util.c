...
int score_letter(char letter, char **vocabulary, size_t num_words) {
  ...
  for (int i = 0; i < (int)num_words; i++) {
    ...
    if (word != NULL && strchr(word, letter)) {
      score++;
    }
  }
...
// strings will need to be freed!x`
...
char getPosition(int position, char *word) {
...
size_t filter_vocabulary_yellow(char letter, int position, char **vocabulary, size_t num_words) {
  ...
  size_t count = 0;
  for (int i = 0; i < (int)num_words; i++) {
    if ((vocabulary[i] != NULL) && (getPosition(position, vocabulary[i]) == letter || strchr(vocabulary[i], letter) == NULL)) {
      free(vocabulary[i]);
      vocabulary[i] = NULL;
      count++;
    }
...
size_t filter_vocabulary_green(char letter, int position, char **vocabulary, size_t num_words) {
  ...
  size_t count = 0; 
  for (int i = 0; i < (int)num_words; i++) {
    if (vocabulary[i] != NULL && getPosition(position, vocabulary[i]) != letter) {
      free(vocabulary[i]);
      vocabulary[i] = NULL;
      count++; 
    }
